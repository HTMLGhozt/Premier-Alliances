// import React, { Fragment } from 'react';
const employmentText = `
    <p>
      For years, Premier Alliances has been one of southeast Arizona’s most 
      prominent employers of individuals with additional needs. Our positions come with flexible hours, full benefits, livable wages, and one-on-one life skills 
      development opportunities so that our employees can work toward 
      achieving their goals.
    </p>
    <p>
      On this page you will find information about any open positions for which 
      we are currently hiring. From here you will also be able to fill out an electronic application and upload your resume.
    </p>
    <p>  
      If there are no positions listed, we are not currently seeking any additional 
      employees at this time. Premier Alliances does not accept applications unless we are actively seeking to fill a position.
    </p>
    <br/>
    <br/>
    <h4>Maintenance Technitian</h4>
    <p>This is example text that will be set in the actual page manager on WordPress</p>
    <h4>Grounds Keeper</h4>
    <p>Continued examples</p>
    <br/>
    <button>Apply</button>`;

const communityText = `
<p>Premier Alliances Inc., in partnership with SourceAmerica®, 
the AbilityOne® Program, and the Federal government, provides fulfilling 
employment opportunities for people with significant disabilities.<p>
<br/>
<p>
In addition, Premier Alliances Inc. gives job placement preference 
to individuals with documented disabilities.</p>
<p>
Premier Alliances posts new job listings frequently and is constantly 
updating the web site with new information.</p>

<h4>Contact our office</h4>
<p> 
to find out how you can be a part of <br/>
the Premier Alliances Team.
</p>
<h5>AbilityOne® Program</h5>
<p>
The AbilityOne Program is a Federal initiative to help people who are blind 
or have other significant disabilities find employment by working within a 
national network of over 600 Nonprofit Agencies that sell products and 
services to the U.S. government. The Committee for Purchase From People Who Are Blind or Severely Disabled is the Federal agency authorized to 
administer the AbilityOne Program.</p>
<br />
<p>Integrity is our highest priority for the AbilityOne Program.</p>
`;

const partnersText = `
<h3>Partnerships Throughout the Community</h3>
<p>Premier Alliances, Inc. is joining forces with local educational institutions, including community colleges and high schools. We offer internships, training and employment opportunities to individuals with significant disabilities. In the future we will work through our partners to offer scholarships for individuals with documented disabilities to attend college or trade school.</p>

<p>Come back to this page periodically to get more details!</p>
<p>
If you are an organization that works with disabled individuals in need of 
employment, training or educational opportunities, please contact us.
</p>
`;
module.exports = {
  employmentText,
  communityText,
  partnersText,
}
